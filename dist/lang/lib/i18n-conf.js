 ...
 *
 * O valor é **derivado automaticamente** do path do próprio arquivo via
 * `import.meta.url`, sem nenhuma configuração manual:
 *
 * - Módulo (`modules/<name>/lang/lib/`) → `'<Name>LangLocales'`
 *   (ex.: `menu` → `'MenuLangLocales'`, `infinite-scroll` → `'InfiniteScrollLangLocales'`)
 * - Core (`<plugin>[-dev]/(src|dist)/lang/lib/`) → `'<PluginName>LangLocales'`
 *   (ex.: `fwk-utilities[-dev]/src/lang/lib/` → `'FwkUtilitiesLangLocales'`)
 *
 * Isso permite copiar `src/lang/` inteiro para qualquer módulo sem nenhum ajuste.
 * Projetos consumidores podem registrar traduções adicionais via `<script>`
 * sem modificar os artefatos do plugin:
 *
 * @example
 * // No HTML, antes de carregar o plugin (core):
 * window.FwkUtilitiesLangLocales = {
 *   'pt-BR': { menu: { mainLabel: 'Menu principal' } }
 * };
 * // Para o módulo Menu:
 * window.MenuLangLocales = { 'pt-BR': { ... } };
 *
 * Usado em:
...
export function getAdapterDictionaries() {
  ...
  const resolveLocale = (locale) => {
    ...
    if (fromCallbackMap) return fromCallbackMap;
    if (callbackResult && !fromCallbackMap) return callbackResult;
